...
function draw() {
    ...
    if (!makeArt) {background(0);}
    // background(100, 200, 50);
    noStroke();
    text(`${balls.length}`, 5, 600);

    fill(0,100,0);
    rect(0,520,1024,717);
    fill(0);
    text("WASD to change ball1 speed -- ", 5, 540);
    text("X to remove ball, Mouse click to add ball, Space to slow balls, Z for some eye candy", 5, 570);
    text(`P to pause  numbers of ball ${balls.length}`, 5, 600);
...
function mousePressed() {
    balls[balls.length] = new ball(50, mouseX, mouseY, random(),random())
